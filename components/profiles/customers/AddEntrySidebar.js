...
export default function AddEntrySidebar({
  ...
  async function handleSubmitClose(closeonsave) {
    ...
    if (errorflag === false) {
      ...
      const response = await fetch("/api/masterdata/customers/create", {
        body: datapass,
        headers: {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-lg p-4 transform transition-transform duration-300 ease-in-out">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold">Add New Client Account</h2>
      ...
      <div className="space-y-4">
        <div>
          <Label htmlFor="itemname">Client Account Name</Label>
          <Input
            id="itemname"
            name="itemname"
            value={newrecordname}
