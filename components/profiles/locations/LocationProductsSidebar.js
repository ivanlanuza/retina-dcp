...
export default function LocationProductsSidebar({ open, mode, item, onClose }) {
  ...
  useEffect(() => {
    if (item) {
      ...
      fetch(
        "/api/locations/getFullLocationProducts?" +
          new URLSearchParams({
  ...
  async function handleSubmit() {
    ...
    const response = await fetch("/api/locations/updateLocationProducts", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
            ...
            <div className="p-6 h-full flex flex-col bg-zinc-50">
              ...
              <div className="flex justify-between items-center mb-8">
                <p className="text-xl font-semibold">
                  Assign Products for {item ? item.name : ""}
                </p>
                <Button
                  variant="ghost"
                  size="icon"
